...
namespace ryu::ide::preferences_editor {

    static logger* s_log = logger_factory::instance()->create(
        "preferences_editor::controller",
        logger::level::info);
    ...
    void controller::bind_events() {
        action_provider().register_handler(
            core::input_action::find_by_name("preferences_editor_leave"),
    ...
    void controller::define_actions() {
        auto leave_action = core::input_action::create_no_map(
            "preferences_editor_leave",
            "Internal",
            "Close the preferences editor and return to previous state.");
    ...
    bool controller::on_load(core::result& result) {
        _layout_panel = core::view_factory::create_loadable_view(
            ...
            result,
            "assets/views/preferences-editor.yaml");
        s_log->result(result);
