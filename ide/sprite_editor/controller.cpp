...
namespace ryu::ide::sprite_editor {

    static logger* s_log = logger_factory::instance()->create(
        "tile_editor::controller",
        logger::level::info);
    ...
    void controller::bind_events() {
        action_provider().register_handler(
            core::input_action::find_by_name("sprite_editor_leave"),
    ...
    void controller::define_actions() {
        auto leave_action = core::input_action::create_no_map(
            "sprite_editor_leave",
            "Internal",
            "Close the sprite editor and return to previous state.");
